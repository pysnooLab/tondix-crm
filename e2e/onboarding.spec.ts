...
test("user onboarding", async ({ page, isMobile, menu, dismissToast }) => {
  ...
  await expect(page).toHaveTitle(/Tondix CRM/);
  await expect(page.getByText("Welcome to Tondix CRM")).toBeVisible();
  ...
  await expect(page.getByText("Install Tondix CRM")).toBeVisible();
