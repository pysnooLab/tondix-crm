...
> Protocole complet dans le skill `agent-team` (workflow, routing, spawn tmux, reflection loop, règles transverses) :
> invoquer avec `Skill({ skill: "agent-team" })` au début d'une session de dispatch.
>
> Le mode silencieux (Playwright headless, Vite sans --open, Vitest sans browser.ui) est enforced automatiquement par `.claude/hooks/silent-mode-check.sh`.
